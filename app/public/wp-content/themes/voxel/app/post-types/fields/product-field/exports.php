	...
	protected function get_dynamic_data_for_product_type( \Voxel\Product_Type $product_type ) {
		...
		if ( $product_type->config( 'modules.addons.enabled' ) ) {
			...
			foreach ( $addons as $addon ) {
				...
				} elseif ( $addon->get_type() === 'numeric' ) {
					$properties[ $addon->get_key() ] = Tag::Object( $addon->get_label() )->properties( function() use ( $addon ) {
						return [
							'label' => Tag::String('Label')->render( function() use ( $addon ) {
								$product_addons = $this->get_product_field('addons');
								if ( ! $product_addons ) {
									return '';
								}
								...
								$product_addon = $product_addons->get_addon( $addon->get_key() );
								if ( ! ( $product_addon && $product_addon->get_type() === 'numeric' && $product_addon->is_active() ) ) {
									return '';
								}

								return $product_addon->get_label();
							} ),
							'price' => Tag::Number('Price')->render( function() use ( $addon ) {
								$product_addons = $this->get_product_field('addons');
								if ( ! $product_addons ) {
									return '';
								}
								...
								$value = $product_addon->get_value();
								return $value['price'] ?? '';
							} ),
							'min' => Tag::Number('Min quantity')->render( function() use ( $addon ) {
								$product_addons = $this->get_product_field('addons');
								if ( ! $product_addons ) {
									return '';
								}
							...
							'max' => Tag::Number('Max quantity')->render( function() use ( $addon ) {
								$product_addons = $this->get_product_field('addons');
								if ( ! $product_addons ) {
									return '';
								}
