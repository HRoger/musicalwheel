...
namespace Voxel\Modules\Stripe_Connect\Widgets;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

class Stripe_Account_Widget extends \Voxel\Widgets\Base_Widget {
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'gen_settings',
			[
				'label' => __( 'General Settings', 'voxel-elementor' ),
			...
			$this->add_control(
				'gen_image',
				[
					'label' => __( 'General image (Stripe Connect)', 'voxel-elementor' ),
		...
		$this->start_controls_section(
			'preview_settings',
			[
				'label' => __( 'Preview Settings', 'voxel-elementor' ),
				'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);
			...
			$this->add_control(
				'preview_as_user',
				[
					'label' => __( 'Preview as user', 'voxel-elementor' ),
					'type' => \Elementor\Controls_Manager::NUMBER,
					'default' => '',
					'min' => 1,
					'step' => 1,
					'description' => 'Enter user ID. Only applies while editing in Elementor. Leave empty to preview as yourself.',
				]
			);
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'ts_stripe_ico',
				[
					'label' => __( 'Stripe icon', 'text-domain' ),
					'type' => \Elementor\Controls_Manager::ICONS,
					'skin' => 'inline',
					'label_block' => false,
				]
			...
			$this->add_control(
				'handle_icon',
				[
					'label' => __( 'Drag handle', 'text-domain' ),
					'type' => \Elementor\Controls_Manager::ICONS,
					'skin' => 'inline',
					'label_block' => false,
				]
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'ts1_field_req_h',
				[
					'label' => __( 'Select/Unselect', 'voxel-elementor' ),
					'type' => \Elementor\Controls_Manager::HEADING,
					'separator' => 'before',
				]
			);
			...
			$this->add_responsive_control(
				...
				[
					...
					'type' => \Elementor\Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .ts-form-group label span a' => 'color: {{VALUE}}',
					],
		...
		$this->start_controls_section(
				...
				[
					'label' => __( 'Form: Select', 'voxel-elementor' ),
					'tab' => \Elementor\Controls_Manager::TAB_STYLE,
				]
			);
					...
					$this->start_controls_tab(
						...
						$this->add_responsive_control(
							...
							[
								...
								'type' => \Elementor\Controls_Manager::COLOR,
								'selectors' => [
									'{{WRAPPER}} .ts-form .ts-filter select' => 'color: {{VALUE}}',
									'{{WRAPPER}} .ts-form .ts-filter select option' => 'color: initial',
									'{{WRAPPER}} .ts-form .ts-filter select optgroup' => 'color: initial',
								],
		...
		$this->start_controls_section(
			'ts_buttons',
			[
				'label' => __( 'Form: Tabs', 'voxel-elementor' ),
				'tab' => \Elementor\Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->start_controls_section(
			'ts_var_buttons',
			[
				'label' => __( 'Form: Pills', 'voxel-elementor' ),
				'tab' => \Elementor\Controls_Manager::TAB_STYLE,
			]
		);
				...
				$this->start_controls_tab(
					...
					$this->add_responsive_control(
						...
						[
							...
							'type' => \Elementor\Controls_Manager::COLOR,
							'selectors' => [
								'{{WRAPPER}} .attribute-select a' => 'color: {{VALUE}}; --ts-icon-color: {{VALUE}}',
							],
	...
	protected function render( $instance = [] ) {
		...
		if ( \Voxel\is_edit_mode() ) {
			$preview_user_id = $this->get_settings_for_display('preview_as_user');
			if ( ! empty( $preview_user_id ) ) {
				$preview_user = \Voxel\User::get( absint( $preview_user_id ) );
				if ( $preview_user ) {
					$user = $preview_user;
					$is_preview = true;
				}
		...
		if ( \Voxel\get( 'payments.stripe.stripe_connect.shipping.responsibility' ) === 'vendor' ) {
			// Get countries with subdivisions and group by continent (same as backend)
			...
			// Get shipping config (zones + rates stored separately)
			$shipping_config = $user->get_vendor_shipping_config();
		...
		require locate_template( 'app/modules/stripe-connect/templates/frontend/stripe-account-widget.php' );
