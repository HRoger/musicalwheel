...
class Listing_Plans_Widget extends \Voxel\Widgets\Base_Widget {
	...
	protected function render( $instance = [] ) {
			...
			foreach ( $group['prices'] as $plan_key ) {
				try {
					...
					$plans[] = [
						...
						'disable_repeat_purchase' => $plan->config('billing.disable_repeat_purchase'),
					];
				} catch ( \Exception $e ) {
		...
		if ( is_user_logged_in() ) {
			...
			// Check which plans have already been purchased by the user (for plans with disable_repeat_purchase)
			$already_purchased_plans = $this->_get_already_purchased_plans( $current_user, $plans );
			foreach ( $plans as &$plan ) {
				$plan['already_purchased'] = in_array( $plan['key'], $already_purchased_plans, true );
			}
			unset( $plan );
		}
	...
	private function _get_already_purchased_plans( $user, $plans ) {
		global $wpdb;

		$plans_to_check = [];
		foreach ( $plans as $plan ) {
			if ( ! empty( $plan['disable_repeat_purchase'] ) ) {
				$plans_to_check[] = $plan['key'];
			}
		}
