...
class Elementor_Controller extends \Voxel\Controllers\Base_Controller {
	...
	$this->on( 'elementor/element/button/section_style/before_section_end', '@custom_button_icon_controls' );
		...
		// fix: page background color not applied to single post templates
		add_filter( 'body_class', function( $classes ) {
			if ( is_singular() ) {
				$post = \Voxel\get_current_post();
				if (
					$post && $post->post_type
					&& $post->post_type->is_managed_by_voxel()
					&& ! $post->is_built_with_elementor()
					&& ( $template_id = \Voxel\get_single_post_template_id( $post->post_type ) )
				) {
					$classes[] = sprintf( 'elementor-page-%d', $template_id );
				}
			}
