...
class Order {
	...
	public function get_actions( \Voxel\User $user ): array {
		...
		return apply_filters( 'voxel/order/actions', $actions, $this );
	}
	...
	public function get_shipping_address(): ?array {
		$details = $this->get_details('shipping.address');
	...
	public function save( $silent = false ): void {
		global $wpdb;
	...
	public static function create_from_cart( \Voxel\Product_Types\Cart\Base_Cart $cart, array $args = [] ): self {
		...
		if ( empty( $items ) ) {
			throw new \Exception( _x( 'No cart items added.', 'cart', 'voxel' ), 108 );
		}

		if ( ! $cart->get_payment_method() ) {
			throw new \Exception( _x( 'No payment method available.', 'cart', 'voxel' ), 109 );
		}

		$payment_methods = Base_Payment_Method::get_all();
		if ( ! isset( $payment_methods[ $cart->get_payment_method() ] ) ) {
			throw new \Exception( _x( 'No payment method available.', 'cart', 'voxel' ), 110 );
		}
		...
		if ( $result === false ) {
			throw new \Exception( _x( 'Could not create order.', 'checkout', 'voxel' ), 115 );
		}
		...
		// skip checkout for zero amount orders
		if (
			$payment_method
			&& ! in_array( $payment_method->get_type(), [
				'zero_amount_payment',
				'zero_amount_subscription'
			], true )
			&& floatval( $payment_method->get_calculated_total() ) === 0.0
		) {
			if ( $payment_method->is_subscription() ) {
				if ( apply_filters( 'voxel/subscriptions/zero_amount/skip_checkout', true ) !== false ) {
					$order->set_details( 'source', [
						'payment_method' => $payment_method->get_type(),
						'is_subscription' => true,
						'billing_cycle' => $payment_method->get_initial_subscription_billing_cycle(),
						'capture_method' => $payment_method->get_capture_method(),
					] );

					$order->set_payment_method('zero_amount_subscription');
					$order->save( silent: true );
				}
			} else {
				if ( apply_filters( 'voxel/payments/zero_amount/skip_checkout', true ) !== false ) {
					$order->set_details( 'source', [
						'payment_method' => $payment_method->get_type(),
						'capture_method' => $payment_method->get_capture_method(),
					] );

					$order->set_payment_method('zero_amount_payment');
					$order->save( silent: true );
				}
			}
		}
