...
function is_importing_elementor_template() {
	...
	if ( ! empty( $GLOBALS['_vx_importing_elementor_template'] ) ) {
		return true;
	}
...
function _is_using_mariadb() {
	...
	// Check if explicitly MariaDB by name
	if ( str_contains( $str1, 'mariadb' ) || str_contains( $str2, 'mariadb' ) || str_contains( $str3, 'mariadb' ) ) {
		return true;
	}

	// Check if MariaDB by version number (10.x, 11.x, etc.)
	// Pattern matches: 10. or 11. followed by one or more digits
	if ( preg_match( '/\b(10|11)\.\d+/', $str1 ) || preg_match( '/\b(10|11)\.\d+/', $str2 ) || preg_match( '/\b(10|11)\.\d+/', $str3 ) ) {
		return true;
	}

	// Default to MySQL if MariaDB is not detected
	return false;
}
...
function get_range_presets( $key = null ) {
	...
	if ( $ranges === null ) {
		$ranges = apply_filters( 'voxel/filters/date/range-presets', [
			...
			'last-24h' => [
				'key' => 'last-24h',
				'label' => _x( 'Last 24h', 'range presets', 'voxel' ),
				'callback' => function( $now ) {
					return [
						'start' => $now->modify( '-24 hours' )->format( 'Y-m-d H:i:s' ),
			...
			'last-week' => [
				'key' => 'last-week',
				'label' => _x( 'Last week', 'range presets', 'voxel' ),
				'callback' => function( $now ) {
					return [
			...
			'last-month' => [
				'key' => 'last-month',
				'label' => _x( 'Last month', 'range presets', 'voxel' ),
				'callback' => function( $now ) {
					return [
...
function enqueue_maps() {
	if ( \Voxel\get('settings.maps.provider') === 'mapbox' && ! \Voxel\is_preview_mode() ) {
		...
		|| ( \Voxel\get( 'settings.maps.provider' ) === 'mapbox' && \Voxel\is_preview_mode() )
	) {
		wp_enqueue_script( 'vx:openstreetmap.js' );
		wp_enqueue_style( 'vx:openstreetmap.css' );
		wp_enqueue_script( 'leaflet' );
		wp_enqueue_style( 'leaflet' );

		// Conditionally load Google Maps library if using Google Maps geocoding
		if ( \Voxel\get('settings.maps.openstreetmap.geocoding_provider') === 'google_maps' ) {
...
function is_zero_decimal_currency( $currency ) {
	return \Voxel\Utils\Currency_List::is_zero_decimal( $currency );
}