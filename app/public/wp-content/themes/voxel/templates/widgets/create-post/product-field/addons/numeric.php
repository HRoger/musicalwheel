		...
		<div v-if="addon.required || value.enabled" class="ts-field-repeater">
			<div class="medium form-field-grid">
				...
				<div class="ts-form-group vx-1-1 switcher-label">
					<label @click.prevent="value.charge_after = value.charge_after || {}; value.charge_after.enabled = !value.charge_after.enabled">
						<div class="switch-slider">
							<div class="onoffswitch">
								<input type="checkbox" class="onoffswitch-checkbox" :checked="value.charge_after?.enabled">
								<label class="onoffswitch-label"></label>
							</div>
						</div>
						<?= _x( 'Charge only after certain quantity', 'product field', 'voxel' ) ?>
					</label>
				</div>
				<div v-if="value.charge_after?.enabled" class="ts-form-group vx-1-1">
					<label>
						<?= _x( 'Free quantity threshold', 'product field', 'voxel' ) ?>
						<div class="vx-dialog">
							<icon-info/>
							<div class="vx-dialog-content min-scroll">
								<p><?= _x( 'Customers will only be charged for quantities above this number. For example, if set to 2, quantities 1-2 are free and charging starts from quantity 3.', 'product field', 'voxel' ) ?></p>
							</div>
						</div>
					</label>
					<div class="input-container">
						<input
							type="number"
							v-model="value.charge_after.quantity"
							class="ts-filter"
							placeholder="<?= esc_attr( _x( 'e.g. 2', 'product field', 'voxel' ) ) ?>"
							min="0"
						>
					</div>
				</div>
