			...
			<template v-for="field in activeRole.fields">
				...
				<template v-else>
					<template v-if="conditionsPass(field)">
						<template v-if="['text','title','profile-name','profile-first-name','profile-last-name'].includes(field.type)">
							<div class="ts-form-group">
								<label>
									{{ field.label }}
						...
						<template v-if="['textarea','description','profile-bio'].includes(field.type)">
							<div class="ts-form-group">
								<label>
									{{ field.label }}
									<span v-if="!field.required" class="is-required"><?= _x( 'Optional', 'auth', 'voxel' ) ?></span>
