			...
			<template v-else>
				<div class="vx-panels">
					...
					<template v-for="provider in props.providers">
						<div class="vx-panel vx-payment-service" :class="['provider-' + provider.key, {active: config.provider === provider.key}]" style="height: 100px;">
							...
							<template v-else-if="provider.key === 'paypal'">
								<div class="panel-image paypal-panel">
									<?php \Voxel\svg( 'paypal.svg' ) ?>
								</div>
							</template>
							<template v-else>
