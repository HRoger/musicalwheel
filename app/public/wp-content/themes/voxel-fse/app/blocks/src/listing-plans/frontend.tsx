/**
 * Listing Plans Block - Frontend Entry Point (Plan C+ Hybrid)
 *
 ...
 *
 * Evidence:
 * - Widget: themes/voxel/app/modules/paid-listings/widgets/listing-plans-widget.php (1856 lines)
 ...
 * ============================================================================
 *
 * Reference: Voxel listing-plans-widget.php (Listing plans VX widget)
 *
 ...
 * ✅ .plan-featured - Featured plan modifier
 * ✅ .ts-plan-featured-text - Featured badge
 ...
 * @returns Normalized ListingPlansVxConfig
 */
 ...
 */
function parseVxConfig(container: HTMLElement): ListingPlansVxConfig | null {
	const vxconfigScript =
		container.querySelector<HTMLScriptElement>('script.vxconfig');
...
async function fetchPlansData(): Promise<ListingPlansApiResponse> {
	...
	return (await response.json()) as ListingPlansApiResponse;
}

/**
 * Wrapper component that handles data fetching
 */
interface ListingPlansWrapperProps {
	config: ListingPlansVxConfig;
}

function ListingPlansWrapper({
	config,
}: ListingPlansWrapperProps): JSX.Element {
	...
	useEffect(() => {
		...
		async function loadPlansData() {
			...
			try {
				...
				if (!cancelled) {
					setError(
						err instanceof Error ? err.message : 'Failed to load plans data'
					);
				}
			} finally {
				if (!cancelled) {
	...
	const attributes: ListingPlansAttributes = {
		blockId: '',
		...
		imageHeight: 0,
		pricingAlign:
			(config.style.pricingAlign as ListingPlansAttributes['pricingAlign']) ??
			'flex-start',
		contentAlign:
			(config.style.contentAlign as ListingPlansAttributes['contentAlign']) ??
			'flex-start',
		descAlign:
			(config.style.descAlign as ListingPlansAttributes['descAlign']) ?? 'left',
		listAlign:
			(config.style.listAlign as ListingPlansAttributes['listAlign']) ??
			'flex-start',
		listGap: 10,
		listIconSize: 18,
		listIconRightPad: 8,
		...
		tabsJustify:
			(config.style.tabsJustify as ListingPlansAttributes['tabsJustify']) ??
			'flex-start',
...
/**
 * Initialize listing plans blocks on the page
 */
function initListingPlansBlocks(): void {
	// Find all listing plans blocks
	const listingPlansBlocks = document.querySelectorAll<HTMLElement>(
	...
	listingPlansBlocks.forEach((container) => {
		...
		if (!config) {
			console.error('No vxconfig found for listing plans block');
			return;
		}
		...
		root.render(<ListingPlansWrapper config={config} />);
	});
}

// Initialize on DOM ready
if (document.readyState === 'loading') {
	document.addEventListener('DOMContentLoaded', initListingPlansBlocks);
} else {
	initListingPlansBlocks();
}

// Also initialize on Turbo/PJAX page loads
window.addEventListener('turbo:load', initListingPlansBlocks);
window.addEventListener('pjax:complete', initListingPlansBlocks);
