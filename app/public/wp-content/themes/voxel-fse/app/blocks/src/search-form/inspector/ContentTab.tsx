						...
						>
							<RepeaterControl< FilterConfig >
								...
								getItemLabel={(filter, index) => {
									let label = filter.filterKey ? filter.filterKey : `Item #${index + 1}`;

									// Add value summary for Range filters
									// Evidence: User request to see price range dynamically in inspector
									if (filter.type === 'range' && filter.defaultValueEnabled && filter.defaultValue && typeof filter.defaultValue === 'string' && filter.defaultValue.includes('..')) {
										const filterData = availableFilters.find(f => f.key === filter.filterKey);
										const props = filterData?.props || {};
										const prefix = props.prefix || '';
										const suffix = props.suffix || '';

										const [min, max] = filter.defaultValue.split('..');

										// Format display similar to frontend range slider
										const displayMin = min ? `${prefix}${min}${suffix}` : (props.min !== undefined ? `${prefix}${props.min}${suffix}` : '');
										const displayMax = max ? `${prefix}${max}${suffix}` : (props.max !== undefined ? `${prefix}${props.max}${suffix}` : '');
								...
								// Persistence: Store expanded filter state in block attributes
								// This survives device changes, viewport switches, and re-renders
								attributes={attributes}
								setAttributes={setAttributes}
