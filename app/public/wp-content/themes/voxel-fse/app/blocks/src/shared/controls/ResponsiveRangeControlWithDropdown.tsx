...
export default function ResponsiveRangeControlWithDropdown({
	...
	// Get WordPress's current device type from the store - this will trigger re-render when viewport changes
	// Using useSelect automatically subscribes to changes, causing component to re-render
	const wpDeviceType = useSelect((select) => {
		...
		return 'Desktop';
	}, []); // Empty dependency array - useSelect handles subscriptions automatically
	...
	const currentUnit = unitAttributeName 
		? (attributes[unitAttributeName] || 'px') as 'px' | '%' | 'em' | 'rem'
		: 'px';

	const setUnit = (unit: 'px' | '%' | 'em' | 'rem') => {
