/**
 * Text Stroke Popup Component
 *
 * Matches Elementor's text stroke control pattern - label with pencil icon that opens a popup
 * with text stroke controls (color, width).
 *
 * Similar to TextShadowPopup but for CSS text-stroke (webkit) property.
 *
 * Features:
 ...
 *
 * Evidence:
 * - Elementor pattern: Group_Control_Text_Stroke
...
interface TextStrokePopupProps {
	label: string;
	attributes: Record<string, any>;
	setAttributes: (attrs: Record<string, any>) => void;
	strokeAttributeName: string;
}
...
export default function TextStrokePopup({
	label,
	attributes,
	setAttributes,
	strokeAttributeName,
}: TextStrokePopupProps) {
	const [isOpen, setIsOpen] = useState(false);
	const popoverRef = useRef<HTMLDivElement>(null);

	// Default text stroke values matching Elementor
	const defaultStroke: TextStrokeValue = {
		color: '#000000',
		width: 1,
	};

	const stroke: TextStrokeValue = attributes[strokeAttributeName] || {};

	// Check if stroke has any custom values (not empty)
	const hasValue = Object.keys(stroke).length > 0;
	...
	const handleOpenPopup = () => {
		if (!isOpen && !hasValue) {
			...
			setAttributes({
				[strokeAttributeName]: { ...defaultStroke },
			});
		}
		setIsOpen(!isOpen);
	};
	...
	return (
		<div className="elementor-control elementor-control-type-text-stroke" style={{ position: 'relative' }}>
			...
			{isOpen && (
				...
				>
					...
					<ColorPickerControl
						...
						value={stroke.color || defaultStroke.color}
						onChange={(color) => {
							setAttributes({
								[strokeAttributeName]: {
									...stroke,
