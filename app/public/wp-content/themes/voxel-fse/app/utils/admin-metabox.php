...
/**
 * Patch Voxel's backend.js to prevent it from interfering with our FSE metabox
 *
 * Instead of dequeuing backend.js (which breaks Author and Expiry metaboxes),
 * we inject scripts that:
 * 1. BEFORE: Remove #vx-fields-wrapper from DOM so backend.js can't find it
 * 2. AFTER: Disable the fieldsMetabox component to prevent AJAX loading
 *
 * The BEFORE script is CRITICAL because backend.js line 57 does:
 *   i.contentWindow.document.querySelector(".ts-create-post").__vue_instance__.submit()
 * This causes errors because our React component doesn't have Vue instances.
 */
function voxel_fse_patch_voxel_backend_scripts()
{
    ...
    // BEFORE script: Remove #vx-fields-wrapper from DOM BEFORE backend.js can find it
    // This prevents backend.js from setting up button click handlers that call __vue_instance__.submit()
    wp_add_inline_script('vx:backend.js', '
		// CRITICAL: Remove Voxel\'s #vx-fields-wrapper BEFORE backend.js processes it
		// This prevents backend.js from setting up competing button handlers
		// that call __vue_instance__.submit() (which fails because we use React, not Vue)
		(function() {
			var voxelWrapper = document.getElementById("vx-fields-wrapper");
			if (voxelWrapper) {
				voxelWrapper.parentNode.removeChild(voxelWrapper);
				//console.log("FSE: Removed #vx-fields-wrapper from DOM to prevent backend.js interference");
			}
		})();
	', 'before');

    // AFTER script: Disable the fieldsMetabox component to prevent AJAX loading
    wp_add_inline_script('vx:backend.js', '
...
function voxel_fse_render_create_post_metabox($wp_post)
{
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function ($) {
            ...
            var clickHandler = null; // Store handler reference for re-attachment
                ...
                if (event.data === 'create-post:mounted') {
                    requestAnimationFrame(function () {
                        ...
                        if (!clickHandlersAttached) {
                            ...
                            // Define click handler in outer scope so it can be re-attached after validation failure
                            clickHandler = function (event) {
                                console.log('[Admin metabox] Button clicked:', event.currentTarget.id);
                                updateButton = event.currentTarget;
                                if (!formSubmitted) {
                                    // Prevent WordPress save and trigger iframe form submission instead
                                    event.preventDefault();
                                    console.log('[Admin metabox] Prevented default, will submit iframe form');
                                    ...
                                    console.log('[Admin metabox] Form element:', formElement);
                                    console.log('[Admin metabox] Has voxelSubmit:', formElement ? !!formElement.voxelSubmit : false);
                                    console.log('[Admin metabox] Has window.voxelFseSubmit:', !!iframe.contentWindow.voxelFseSubmit);

                                    // Try DOM element method first
                                    if (formElement && formElement.voxelSubmit) {
                                        console.log('[Admin metabox] Calling formElement.voxelSubmit()');
                                        formElement.voxelSubmit();
                                        submitCalled = true;
                                    }
                                    // Fallback to window method
                                    else if (iframe.contentWindow.voxelFseSubmit) {
                                        console.log('[Admin metabox] Calling iframe.contentWindow.voxelFseSubmit()');
                                    ...
                                    console.log('[Admin metabox] Form already submitted, allowing default action');
                                }
                            };

                            // Attach handler to all buttons
                            var buttons = document.querySelectorAll('#publish, #save-post');
                            console.log('[Admin metabox] Found buttons:', buttons.length, buttons);
                            Array.from(buttons).forEach(function (button) {
                                console.log('[Admin metabox] Attaching click handler to:', button.id);
...
/**
 * Intercept Voxel's original admin.get_fields_form AJAX action
 *
 * Since we've replaced Voxel's metabox with our FSE version, we need to intercept
 * Voxel's original AJAX call and return a success response to prevent 500 errors.
 *
 * The JavaScript patch (voxel_fse_patch_voxel_backend_scripts) has timing issues
 * where Vue mounts before the patch runs, so Voxel's AJAX call still fires.
 *
 * By hooking into the same action with HIGHER priority (lower number = earlier execution),
 * we can return a minimal success response and prevent Voxel's handler from running.
 */
add_action('voxel_ajax_admin.get_fields_form', 'voxel_fse_intercept_voxel_metabox_ajax', 5);

function voxel_fse_intercept_voxel_metabox_ajax()
{
    // Return a minimal HTML response that tells the iframe it's handled
    // This prevents Voxel's handler from running and causing a 500 error
    ?>
    <!DOCTYPE html>
    <html>
    <head><title>FSE Handled</title></head>
    <body style="margin:0;padding:20px;font-family:sans-serif;">
    <p style="color:#666;">This metabox is handled by the Voxel FSE child theme.</p>
    </body>
    </html>
    <?php
    exit;
