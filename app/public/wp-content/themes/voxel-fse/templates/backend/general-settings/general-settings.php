<?php
/**
 * Admin general settings - Child Theme Override
 *
 * Added Courses addon module
 *
 * @since 1.0
 */
	...
	<form method="POST" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ) ?>" @submit="state.submit_config = JSON.stringify( config )">
		<div class="sticky-top">
			<div class="vx-head x-container">
				...
				<h2 v-else-if="tab === 'courses'">Courses</h2>
			...
			<div class="x-row">
				<div class="x-col-3">
					<ul class="inner-tabs vertical-tabs">
						...
						<li v-if="config.addons.courses && config.addons.courses.enabled" :class="{'current-item': tab === 'courses'}">
							<a href="#" @click.prevent="setTab('courses')">Courses</a>
						</li>
				...
				<div v-if="tab === 'addons'" class="x-col-9">
					<div class="vx-panels">
						...
						<!-- NEW: Courses Addon -->
						<div class="vx-panel">
							<div class="panel-image addon-courses">
								<?php
								$child_icon = get_stylesheet_directory() . '/assets/images/svgs/graduation-cap.svg';
								if ( file_exists( $child_icon ) ) {
									include $child_icon;
								} else {
									include get_template_directory() . '/assets/images/svgs/file.svg';
								}
								?>
							</div>
							<div class="panel-info">
								<h3>Courses</h3>
								<ul>
									<li>Create and manage online courses with lessons, assignments, and student progress tracking</li>
								</ul>
							</div>
							<div class="panel-switcher">
								<?php \Voxel\Utils\Form_Models\Switcher_Model::render( [
									'v-model' => 'config.addons.courses.enabled',
									'classes' => 'x-col-12',
								] ) ?>
							</div>
						</div>
				...
				<div v-else-if="tab === 'courses'" class="x-col-9">
					<?php
					// Load Courses settings page (will create this next)
					$courses_settings = locate_template( 'templates/backend/general-settings/courses-settings.php' );
					if ( $courses_settings ) {
						require_once $courses_settings;
					} else {
						?>
						<div class="ts-group">
							<div class="ts-group-head">
								<h3>Courses Settings</h3>
							</div>
							<div class="x-row">
								<div class="x-col-12">
									<p>Courses addon is enabled. Configure your course settings here.</p>
									<p style="color: #999; font-style: italic;">Course settings interface coming soon...</p>
								</div>
							</div>
						</div>
						<?php
