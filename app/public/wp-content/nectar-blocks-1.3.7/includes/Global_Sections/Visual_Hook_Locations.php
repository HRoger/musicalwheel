...
class Visual_Hook_Locations
{
    ...
    public function display_hook_locations() 
    {
        ...
        $nectar_hooks = Display_Options::get_instance()->theme_hooks;

        foreach( $nectar_hooks as $category ) {
            foreach( $category as $hook ) {
                if( isset($hook['value']) ) {
                    ...
                    add_action( $hook, function() use ($label, $hook) {
                        ...
                        $add_new_button = '<a target="_blank" rel="noreferrer" href="' . admin_url('post-new.php?post_type=nectar_sections&nectar_starting_hook=' . esc_html($hook)) . '"><i class="fa fa-plus-circle"></i> <span>' . esc_html__('Add New Global Section', 'nectar-blocks') . '</span></a>';
                        $row_shortcode = '<div class="nectar-blocks-hook-location__content"><span>' . esc_html__('Hook:', 'nectar-blocks') . ' ' . esc_html($label) . '</span>' . $add_new_button . '</div>';
