	...
	public static function execute( $params ) {
		...
		if ( isset( $multisite['Sites'] ) && ( $subsites = $multisite['Sites'] ) ) {
			foreach ( $subsites as $i => $subsite ) {
				...
				// Get old blog path
				$old_blog_path = strval( parse_url( $config['HomeURL'], PHP_URL_PATH ) );

				// Get new blog domain
				$new_blog_domain = parse_url( home_url(), PHP_URL_HOST );

				// Get new blog path
				$new_blog_path = strval( parse_url( home_url(), PHP_URL_PATH ) );
		...
		if ( isset( $params['options']['sites'] ) && ( $subsites = $params['options']['sites'] ) ) {
			...
			foreach ( $subsites as $i => $url ) {
				...
				// Get blog path
				$blog_path = strval( parse_url( trim( $url ), PHP_URL_PATH ) );
