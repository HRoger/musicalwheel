	...
	public static function execute( $params ) {
		...
		if ( empty( $multisite['Network'] ) ) {
			...
			if ( $blogs ) {
				...
				// Update user meta
				$db_client->query( "UPDATE {$mainsite_prefix}usermeta SET meta_key = REPLACE(meta_key, '{$mainsite_prefix}', '{$base_prefix}')" );
				...
				$db_client->query(
					...
					INNER JOIN
						{$base_prefix}users AS u ON (mu.user_login COLLATE utf8mb4_unicode_520_ci = u.user_login COLLATE utf8mb4_unicode_520_ci)
					LEFT JOIN
						{$base_prefix}usermeta AS um ON (um.user_id = u.ID AND mum.meta_key COLLATE utf8mb4_unicode_520_ci = um.meta_key COLLATE utf8mb4_unicode_520_ci)
					WHERE
						um.meta_key IS NULL"
				);
			}
