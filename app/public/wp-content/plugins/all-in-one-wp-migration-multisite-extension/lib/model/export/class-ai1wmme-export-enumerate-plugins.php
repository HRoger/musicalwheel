...
class Ai1wmme_Export_Enumerate_Plugins {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Retrieving a list of WordPress plugin files...', AI1WMME_PLUGIN_NAME ) );
				...
				foreach ( ai1wmme_get_sites( $params ) as $site ) {
					...
					foreach ( get_plugins() as $plugin_name => $plugin_info ) {
						if ( is_plugin_active( $plugin_name ) ) {
							$active_plugins[ $plugin_name ] = $plugin_name;
						}
					}
		...
		// Set progress
		Ai1wm_Status::info( __( 'Done retrieving a list of WordPress plugin files.', AI1WMME_PLUGIN_NAME ) );
