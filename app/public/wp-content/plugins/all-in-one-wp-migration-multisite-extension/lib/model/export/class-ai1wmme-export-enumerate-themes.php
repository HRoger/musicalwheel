...
class Ai1wmme_Export_Enumerate_Themes {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Retrieving a list of WordPress theme files...', AI1WMME_PLUGIN_NAME ) );
		...
		if ( isset( $params['options']['no_inactive_themes'] ) ) {
			...
			if ( isset( $params['options']['sites'] ) ) {
				foreach ( ai1wmme_get_sites( $params ) as $site ) {
					...
					// Add active theme
					foreach ( array( get_template(), get_stylesheet() ) as $theme_name ) {
						$active_themes[ $theme_name ] = $theme_name;
					}
		...
		// Set progress
		Ai1wm_Status::info( __( 'Done retrieving a list of WordPress theme files.', AI1WMME_PLUGIN_NAME ) );
