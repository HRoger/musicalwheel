...
class Global_Typography extends Settings_Base {
  public static $OPTION_NAME = 'nectar_global_typography';
  ...
  */
  public static function get_line_height_rules(array $settings, string $selectors) {
    $css = '';
    foreach( $settings['lineHeight'] as $device => $line_height ) {

      // Log::info("line-height: " . $device, $line_height);
  ...
  public static function get_font_size_rules(array $settings, string $selectors) {
    ...
    foreach( $settings['fontSize'] as $device => $size ) {
      ...
      // Log::info("font-size: " . $device, $size);
