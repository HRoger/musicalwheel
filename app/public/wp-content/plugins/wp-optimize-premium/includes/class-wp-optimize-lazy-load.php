...
class WP_Optimize_Lazy_Load {
	...
	/**
	 * Try to retrieve the image blob, false if url returns no blob.
	 *
	 * @param string $video_id
	...
	private function remote_get_youtube_thumbnail_image(string $video_id, string $resolution) {
		...
		if (200 === wp_remote_retrieve_response_code($response)) {
			...
			if (! @file_put_contents(WPO_CACHE_YOUTUBE_THUMBNAILS_DIR . '/' . $file_name . '.webp', $blob)) { // phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged -- suppress the warning due to network issues
				return false;
			}
			return WPO_CACHE_YOUTUBE_THUMBNAILS_URL . '/' . $file_name . '.webp';
		}
		...
		$blob = wp_remote_retrieve_body($response);
		if (! @file_put_contents(WPO_CACHE_YOUTUBE_THUMBNAILS_DIR . '/' . $file_name . '.jpg', $blob)) { // phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged -- suppress the warning due to network issues
			return false;
		}

		return WPO_CACHE_YOUTUBE_THUMBNAILS_URL . '/' . $file_name . '.jpg';
	}
	...
	private function download_preview_image(string $video_id) {
		...
		$resolution = in_array($filter_resolution, $accepted_resolutions) ? $filter_resolution : 'maxresdefault';

		$maybe_cached_file = $this->get_cached_image_file($video_id, $resolution);
		if ($maybe_cached_file) return $maybe_cached_file;

		$maybe_preview_image = $this->remote_get_youtube_thumbnail_image($video_id, $resolution);
		if ($maybe_preview_image) return $maybe_preview_image;

		//do iterative logic for other resolution if the selected is not found
		$other_resolutions = array_diff($accepted_resolutions, array($resolution));
		// Check if we already cached any of the other resolutions
		$maybe_matching_cached_file = $this->get_matching_cached_image_file($video_id, $other_resolutions);
		if ($maybe_matching_cached_file) return $maybe_matching_cached_file;

		foreach ($other_resolutions as $res) {
			$maybe_preview_image = $this->remote_get_youtube_thumbnail_image($video_id, $res);
			if ($maybe_preview_image) return $maybe_preview_image;
		}
		return false;
	}
	 ...
	 * @return string | false
	 */
	private function get_cached_image_file(string $video_id, string $resolution) {
		$file_name = $video_id . '-' . $resolution;
		$downloaded_webp_image_path = WPO_CACHE_YOUTUBE_THUMBNAILS_DIR . '/' . $file_name . '.webp';
		if (file_exists($downloaded_webp_image_path)) return WPO_CACHE_YOUTUBE_THUMBNAILS_URL . '/' . $file_name . '.webp';

		$downloaded_jpg_image_path = WPO_CACHE_YOUTUBE_THUMBNAILS_DIR . '/' . $file_name . '.jpg';
		if (file_exists($downloaded_jpg_image_path)) return WPO_CACHE_YOUTUBE_THUMBNAILS_URL . '/' . $file_name . '.jpg';

		return false;
	}

	/**
	 * Searches for a matching cached preview image for the specified video id or false if it does not exist yet.
	 *
	 * @param string $video_id
	 ...
	 * @return string | false
	 */
	private function get_matching_cached_image_file(string $video_id, array $resolutions) {
		foreach ($resolutions as $res) {
			$maybe_cached_file = $this->get_cached_image_file($video_id, $res);
			if ($maybe_cached_file) return $maybe_cached_file;
		}
		return false;
	}
